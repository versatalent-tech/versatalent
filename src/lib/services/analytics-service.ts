...
export class AnalyticsService {
  ...
  static calculateTrends(current: AnalyticsMetrics, previous: AnalyticsMetrics): Record<string, number> {
  ...
  static async broadcastRealtimeEvent(talentId: string, eventType: string, data: Record<string, unknown>): Promise<void> {
  ...
  static async getPerformanceMetrics(talentId: string): Promise<Record<string, unknown>> {
    const db = this.db;
  ...
  static async exportAnalyticsData(talentId: string, filters: AnalyticsFilters, format: 'json' | 'csv' = 'json'): Promise<Record<string, unknown> | string> {
  ...
  private static convertToCSV(data: Record<string, unknown>): string {
    ...
    const rows = events.map((event: Record<string, unknown>) => [
      ...
      event.timestamp,
      event.sessionId,
      (event.deviceInfo as Record<string, unknown>)?.deviceType,
      (event.locationInfo as Record<string, unknown>)?.country,
      (event.referrerInfo as Record<string, unknown>)?.source
    ]);
  ...
  private static calculateTrafficSources(events: AnalyticsEvent[]): TrafficSources {
    ...
    return {
      sources: Object.entries(sourceCounts).map(([source, count]) => ({
        source: source as 'direct' | 'social' | 'search' | 'referral',
        visits: count,
        percentage: Math.round((count / total) * 100),
  ...
  static subscribeToRealtimeUpdates(talentId: string, callback: (data: Record<string, unknown>) => void): () => void {
