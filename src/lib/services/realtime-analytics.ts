  ...
  useEffect(() => {
    ...
    const subscription: RealtimeSubscription = {
      ...
      callback: (event: RealtimeEvent) => {
        ...
        switch (event.type) {
          case 'new_event':
            setRealtimeData((prev: Record<string, unknown> | null) => ({
              ...prev,
...
async function fetchInitialRealtimeData(talentId: string): Promise<Record<string, unknown> | null> {
  try {
...
export function useVisitorTracking(talentId: string) {
  ...
  const [recentVisitors, setRecentVisitors] = useState<Record<string, unknown>[]>([]);
