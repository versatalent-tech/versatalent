...
export class AnalyticsDatabase {
  ...
  async getEvents(filters: Partial<AnalyticsFilters & { limit: number }>): Promise<AnalyticsEvent[]> {
    ...
    if (filters.countries) {
      events = events.filter(e => {
        const location = JSON.parse(e.locationInfo as any);
    ...
    if (filters.deviceTypes) {
      events = events.filter(e => {
        const device = JSON.parse(e.deviceInfo as any);
  ...
  subscribeToRealtimeUpdates(talentId: string, callback: (data: any) => void): () => void {
  ...
  private notifyRealtimeSubscribers(talentId: string, data: any): void {
    const subscribers = this.realtimeSubscribers.get(talentId);
    if (subscribers) {
